    ...
    <div class="sheet-body">
      ...
      <div class="import-form">
        ...
        <div v-if="resultadoImportacao" class="resultado">
          <div class="resultado-header" :class="resultadoImportacao.importadosCount > 0 ? 'success' : 'warning'">
            <span v-if="resultadoImportacao.importadosCount > 0">
              ✓ {{ resultadoImportacao.importadosCount }} lançamento(s) importado(s)
            </span>
            <span v-else>
...
async function importarTexto() {
  ...
  try {
    ...
    resultadoImportacao.value = {
      ...
      importadosCount: resumo.importadosCount,
      totalImportado: resumo.totalImportado
    }

    if (resumo.importadosCount > 0) {
      s.notify(`${resumo.importadosCount} lançamento(s) importado(s) com sucesso!`, 'success', 3000)
      textoExtrato.value = ''
